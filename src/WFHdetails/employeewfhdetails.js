...
class MyWfhRequest extends Component {
  constructor(props) {
  ...
  submitComment = event => {
    ...
    if (this.state.userRole == 'Employee') {
      Auth.submitWFHEmployeeComment(idTestq, this.refs.chatRec.value).then(data => {
      ...
      Auth.submitWFHManagerComment(idTestq, this.refs.chatRec.value).then(data => {
  ...
  handleChange(e) {
    ...
    if (divPending == true) {
      Auth.searchByWfhDetails(x).then(response => {
        var pendingData = [];
        var sortedLeaves = [];
        var pendingResponseData = [];
        if (response.wfh != undefined) {
          for (var i = 0; i < response.wfh.length; i++) {
            if ((response.wfh[i].start_date && response.wfh[i].end_date) != undefined) {
            ...
            if (response.wfh[i].approval_status == "Pending") {
              pendingData.push(response.wfh[i]);
            }
          }
        ...
        if (response.wfh != undefined) {
          ...
          for (var i = 0; i < response.wfh.length; i++) {
            ...
            if (response.wfh[i].approval_status == "Approved") {
              approvedData.push(response.wfh[i]);
            }
          }
          this.setState({
        ...
        if (response.wfh != undefined) {
          ...
          for (var i = 0; i < response.wfh.length; i++) {
            ...
            if (response.wfh[i].approval_status == "Rejected") {
              rejectedData.push(response.wfh[i]);
            }
          }
          this.setState({
  ...
  wfhdetail.classList.add("active");
    ...
    Auth.getWFHDetail().then(response => {
      ...
      if (response.wfh != undefined) {
        ...
        for (var i = 0; i < response.wfh.length; i++) {
          ...
          if (response.wfh[i].approval_status == "Rejected") {
            declinedData.push(response.wfh[i]);
          }
          if (response.wfh[i].approval_status == "Pending") {
            pendingData.push(response.wfh[i]);
          }
                                          ...
                                          <div >
                                            {item.message == "Approved By Delivery Head" ? (
                                              <div className="hr-mang-ld">
                                                ...
                                                <div className="hr-mang-ld">
                                                  <div className=" tech btn-default m-r-20 leave-details btn-rounded">
                                                    Tech
                                    </div>
                                                  <div className=" hr-mang btn-default btn-rounded">
                                                    HR
